...
class BaseModel(ABC):
    ...
    def __init__(self, opt):
        ...
        self.newstage_train = opt.stage2_checkpoints
    ...
    def load_networks(self, epoch):
        ...
        opt = '%s_optimizer_%s.pth' % (epoch)
        load_opt_path = torch.load(os.path.join(self.save_dir, opt))
        self.optimizer_G = torch.load_state_dict(load_opt_path['G_optimizer'])
        self.optimizer_D = torch.load_state_dict(load_opt_path['D_optimizer'])
    
    
    def load_stage2_networks(self, epoch):
    ...
    def load_partial_networks(self, epoch):
        ...
        pretrained_nets = ['G_SH_encoder', 'G_SS_encoder', 'G_GS_encoder', 'G_GH_encoder', 'D_A', 'D_B', 'D_C', 'D_D', 'G_SH_decoder', 'G_SS_decoder', 'G_GS_decoder', 'G_GH_decoder', 
                           'G_SH_decoder', 'G_SS_decoder', 'G_GS_decoder', 'G_GH_decoder']
        for name in self.model_names:
    ...
    def average_pretrained_weights(self, model_type):
        ...
        config = {"B50f_encoder": os.path.join(self.newstage_train, "100_net_G_SH_encoder.pth"),
          "B30f_encoder":os.path.join(self.newstage_train,"100_net_G_SS_encoder.pth"),
          "BONE_encoder":os.path.join(self.newstage_train,"100_net_G_GH_encoder.pth"),
          "STD_encoder":os.path.join(self.newstage_train,"100_net_G_GS_encoder.pth"),
          "B30f_decoder":os.path.join(self.newstage_train,"100_net_G_SS_decoder.pth"),
          "STD_decoder":os.path.join(self.newstage_train,"100_net_G_GS_decoder.pth"),
          "B50f_decoder":os.path.join(self.newstage_train,"100_net_G_SH_decoder.pth"),
          "BONE_decoder":os.path.join(self.newstage_train,"100_net_G_GH_decoder.pth"),
          "D_A":os.path.join(self.newstage_train,"100_net_D_A.pth"),
          "D_B":os.path.join(self.newstage_train,"100_net_D_B.pth"),
          "D_C":os.path.join(self.newstage_train,"100_net_D_C.pth"),
          "D_D":os.path.join(self.newstage_train,"100_net_D_D.pth")
        }
        if model_type == "encoder":
            m1 = torch.load(config['B50f_encoder'])
            m2 = torch.load(config['B30f_encoder'])
            m3 = torch.load(config['BONE_encoder'])
            m4 = torch.load(config['STD_encoder'])
        elif model_type == "decoder":
            m1 = torch.load(config['B50f_decoder'])
            m2 = torch.load(config['B30f_decoder'])
            m3 = torch.load(config['BONE_decoder'])
            m4 = torch.load(config['STD_decoder'])
        elif model_type == 'discriminator':
            m1 = torch.load(config['D_A'])
            m2 = torch.load(config['D_B'])
            m3 = torch.load(config['D_C'])
            m4 = torch.load(config['D_D'])

        M_avg = OrderedDict()
        for k in m1.keys():
            M_avg[k] = (m1[k] + m2[k] + m3[k] + m4[k])/4.0
        return M_avg